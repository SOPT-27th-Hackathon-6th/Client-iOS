...
class StampCell: UICollectionViewCell {
    ...
    ///    }
    func settingCell(isMara : Bool, count : Int,isFilled : Bool)
    {
        if isMara == true // 마라에서
        {
                ...
                if isFilled == true
                {
                    plateImageView.image = UIImage(named: "malaStampStamped")
                }
                else
                {
        ...
        else // 국밥의 경우
        {
                ...
                {
                    plateImageView.image = UIImage(named: "gukbabStampUnstamped")
                }
